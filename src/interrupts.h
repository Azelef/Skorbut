...
#define INTERRUPTS_BASE (PERIPHERIAL_BASE + INTERRUPTS_OFFSET)
#define INTERRUPTS_PENDING (INTERRUPTS_BASE + 0x200)

#define IRQ_IS_BASIC(x) ((x >= 64))
#define IRQ_IS_GPU2(x) ((x < 64 && x >= 32))
...
typedef void (*interrupt_clearer_f)(void);//avant de traiter une interrupt
